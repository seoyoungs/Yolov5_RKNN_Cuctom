...
def export_saved_model(model, im, file, dynamic,
    ...
    try:
        ...
        im = tf.zeros((batch_size, *imgsz, 4))  # BHWC order for TensorFlow
        y = tf_model.predict(im, tf_nms, agnostic_nms, topk_per_class, topk_all, iou_thres, conf_thres)
        inputs = keras.Input(shape=(*imgsz, 4), batch_size=None if dynamic else batch_size)
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    if isinstance(model.model[-1], Detect):
        ...
        RK_anchors = model.model[-1].stride.reshape(4,1).repeat(1,3).reshape(-1,1)* model.model[-1].anchors.reshape(12,2) # repeat(1,3), anchors.reshape(9,2)
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128_custom_crowd.yaml', help='dataset.yaml path')
