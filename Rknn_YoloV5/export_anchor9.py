...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    im = torch.zeros(batch_size, , *imgsz).to(device)  # image size(1,3,320,192) BCHW iDetection
    ...
    if isinstance(model.model[-1], Detect):
        ...
        RK_anchors = model.model[-1].stride.reshape(3,1).repeat(1,3).reshape(-1,1)* model.model[-1].anchors.reshape(9,2) # 9,2
